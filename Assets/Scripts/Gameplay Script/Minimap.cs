...
public class Minimap : MonoBehaviour
{
    ...
    public List<Transform> waypoints = new List<Transform>();              // Waypoints for the line renderer
	...
	public List<MeshRenderer> wayPointsMesh = new List<MeshRenderer>();
	public List<BoxCollider> wayPointsBCol = new List<BoxCollider>();
    ...
    void DrawRoute()
    {
        ...
        if (lineRenderer != null && waypoints.Count > 0)
        {
			...
			foreach (MeshRenderer WPM in wayPointsMesh)
			{
				WPM.enabled = false;
			}

			wayPointsMesh[0].enabled = true;

			foreach (BoxCollider WPBC in wayPointsBCol)
			{
				WPBC.enabled = false;
			}

			wayPointsBCol[0].enabled = true;
	...
	public void LineRendererManualUpdate()
	{
		if (lineRenderer != null && waypoints.Count > 0)
		{
			// Set the number of points in the LineRenderer
